...
## 10. Phase 11 — Client Foundation Additions

Phase 10 had already built most of the "foundation" Phase 11 asks for: the `core/`/`shared/`/`features/` layering (§2), the centralized `ApiClient`/repositories (§8's data layer), `Env` (§6's configuration mechanism), the typed models (§ models), and `AuthController`/`AuthGateway` (§5's authentication foundation). Phase 11 confirmed that layering is unchanged and added the two pieces genuinely missing from it: a named-route table and a responsive layout switch.

- **`lib/app/routes.dart`** — `AppRoutes`, a named-route table (`login`, `dashboard`, `subjects`, `subjectDetail`, `lectureDetail`, `profile`) with `generateRoute(RouteSettings)`. This is additive: it does not replace the existing `Navigator.push`/`MaterialPageRoute` flow screens already use to navigate between each other, and it does not replace `AuthGate` (`app.dart`) as the app's actual startup auth wall — that gate still decides what `MaterialApp.home` renders. `AppRoutes.protected` marks every route except `login` as requiring `AuthStatus.authenticated`; a protected route requested while unauthenticated resolves to `LoginScreen` instead of the protected screen, mirroring `AuthGate`'s behavior for anything reached via `Navigator.pushNamed` in the future. As before, the backend re-verifies every request regardless (`AUTHORIZATION.md`) — this route guard is convenience/UX, not a security boundary.
- **`lib/core/responsive/breakpoints.dart`** — `Breakpoints`, pure width-based classification (`isMobileWidth`/`isTabletWidth`/`isDesktopWidth`, plus `BuildContext` convenience wrappers) at 600px/1024px thresholds. `RootShell` (`lib/app/root_shell.dart`) uses this to switch its navigation chrome between a bottom `NavigationBar` (mobile) and a side `NavigationRail` (tablet/desktop) while keeping the same four/five tabs and the same selected-tab state — one Flutter codebase, not three separate applications (Phase 11 §4H, §7).

Both additions are covered by `test/unit/breakpoints_test.dart` (pure width-threshold logic) and `test/widget/routes_test.dart` (unknown-route handling, the protected-route guard, and that all six required route names are registered) — see `MOBILE_TEST_PLAN.md` for their verification status in this environment.
