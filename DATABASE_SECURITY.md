...
- **`subjects` / `lectures` / `lecture_items` / `quizzes` / `question_banks` / `questions` / `question_options`**: `select` where `status = 'published'` for any authenticated session, **or** caller is `admin` (who may also see `draft`). All writes via backend service role only.
